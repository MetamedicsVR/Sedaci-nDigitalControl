...
public class AnimationHierarchyEditor : EditorWindow {
	...
	void ApplyAllChanges() {
		foreach (var entry in tempPathOverrides) {
			...
			for (int iCurrentPath = 0; iCurrentPath < pathsKeys.Count; iCurrentPath++) {
				...
				for (int i = 0; i < curves.Count; i++) {
					...
					if (path.Contains(sReplacementOldRoot)) {
						...
						if (string.IsNullOrWhiteSpace(sReplacementNewRoot)) {
							sNewPath = Regex.Replace(path, "^" + sReplacementOldRoot, "");
						} else {
							sNewPath = Regex.Replace(path, "^" + sReplacementOldRoot, sReplacementNewRoot);
						}
